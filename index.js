...
app.post("/products", authenticateToken, (req, res) => {
  ...
  if (typeof category !== "string" || category.trim() === "") {
    return res
      .status(400)
      .json({ message: "Invalid or missing 'category' field" });
  }
  if (typeof price !== "number" || price <= 0) {
    return res
      .status(400)
      .json({ message: "Invalid 'price' field, must be a positive number" });
  }
  if (typeof description !== "string" || description.trim() === "") {
    return res
      .status(400)
      .json({ message: "Invalid or missing 'description' field" });
  }
  if (!image) {
  ...
  if (typeof rating !== "number" || rating < 0 || rating > 5) {
    return res
      .status(400)
      .json({ message: "Invalid 'rating' field, must be between 0 and 5" });
  }
...
app.post("/orders", (req, res) => {
  ...
  for (const item of items) {
    ...
    if (typeof item.quantity !== "number" || item.quantity <= 0) {
      return res
        .status(400)
        .json({ message: "Each item 'quantity' should be a positive number." });
    }
  }
